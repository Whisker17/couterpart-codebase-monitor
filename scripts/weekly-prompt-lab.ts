/**
 * Weekly report prompt lab.
 *
 * Usage:
 *   bun run scripts/weekly-prompt-lab.ts
 *   bun run scripts/weekly-prompt-lab.ts --prompt prompts/reports/weekly/baseline.md --prompt prompts/reports/weekly/action-oriented.md
 *   bun run scripts/weekly-prompt-lab.ts --date 2026-06-08 --out data/reports/prompt-lab
 *   bun run scripts/weekly-prompt-lab.ts --dry-run
 *
 * Reads existing DB analyses for the past 7-day report window, runs one or
 * more weekly synthesis prompts against the exact same input, and writes local
 * comparison artifacts. It does not write reports/report_deliveries and does
 * not dispatch to Lark.
 */
...
} from "../src/extensions/report-generator/weekly-prompt-input";

const DEFAULT_PROMPT_DIR = "prompts/reports/weekly";
const DEFAULT_OUTPUT_ROOT = "data/reports/prompt-lab";

export interface WeeklyPromptLabCliOptions {
  promptPaths: string[];
  outputRoot: string;
  date?: string;
  timezone?: string;
...
export interface WeeklyPromptLabOptions extends WeeklyPromptLabCliOptions {
  now?: Date;
}
...
export interface WeeklyPromptLabResult {
  outputDir: string;
...
export type GenerateWeeklyTextFn = (options: GeneratePromptOptions) => Promise<GeneratedText>;

export interface WeeklyPromptLabDeps {
  db?: Database;
  generateFn?: GenerateWeeklyTextFn;
}

function usage(): string {
  return `Usage:
  bun run scripts/weekly-prompt-lab.ts [options]

Options:
  --prompt <path>             Prompt file or directory. Repeatable. Defaults to ${DEFAULT_PROMPT_DIR}
  --date <YYYY-MM-DD>         Anchor date for the 7-day weekly window. Defaults to now
...
export function parseArgs(argv: string[]): WeeklyPromptLabCliOptions {
  const opts: WeeklyPromptLabCliOptions = {
    promptPaths: [],
...
export { buildWeeklyPromptInput, renderWeeklyPrompt as renderPrompt };
...
export async function runWeeklyPromptLab(
  options: WeeklyPromptLabOptions,
  deps: WeeklyPromptLabDeps = {}
): Promise<WeeklyPromptLabResult> {
  ...
  const input = buildWeeklyPromptInput(db, timezone, now);

  const promptFiles = await resolvePromptFiles(options.promptPaths);
  const outputDir = join(
    options.outputRoot,
    `weekly-${input.period.endDate}-${options.runId ?? timestampRunId()}`
  );
  ...
  for (const promptPath of promptFiles) {
    ...
    console.log(
      `[WeeklyPromptLab] ${options.dryRun ? "Rendering" : "Generating"} ${name} from ${promptPath}`
    );
    const variantDir = join(outputDir, name);
    await mkdir(variantDir, { recursive: true });

    const template = await readFile(promptPath, "utf-8");
    const prompt = renderWeeklyPrompt(template, input);
...
async function main(): Promise<number> {
  let options: WeeklyPromptLabCliOptions;
  try {
    ...
    console.error(`[WeeklyPromptLab] ${err instanceof Error ? err.message : String(err)}`);
    console.error(usage());
    return 1;
  }

  try {
    const result = await runWeeklyPromptLab(options);
    console.log(`[WeeklyPromptLab] Wrote ${result.variants.length} variant(s) to ${result.outputDir}`);
    ...
    console.error(`[WeeklyPromptLab] Failed: ${err instanceof Error ? err.message : String(err)}`);
    return 1;
  } finally {
    closeDb();
  }
