...
function getDailyDigestCoverage(
  db: Database,
  startUnix: number,
  endUnix: number
): WeeklyPromptInput["dailyDigestCoverage"] {
  try {
    const rows = db
      .query<DigestCoverageRow, [number, number]>(
    ...
    return {
      present,
      nullDigest,
      missing: Math.max(0, 7 - rows.length),
    };
  } catch {
    return { present: 0, nullDigest: 0, missing: 7 };
  }
}

export function buildWeeklyPromptInput(
  db: Database,
  timezone: string,
  now: Date = new Date()
): WeeklyPromptInput {
  const { startUnix, endUnix } = getWeekPeriod(timezone, now);
  const startDate = formatDate(startUnix, timezone);
  const endDate = formatDate(endUnix, timezone);
  ...
  const projectMap = new Map<string, WeeklyPromptInput["projects"][number]["prs"]>();
  ...
  const projects: WeeklyPromptInput["projects"] = [];
  const directionChanges: WeeklyPromptInput["directionChanges"] = [];
  ...
  for (const [projectId, prs] of projectMap.entries()) {
    ...
    const topSignals = uniqueStrings(prs.map((pr) => pr.directionSignal));
    ...
    if (projectDirectional.length > 0) {
      directionChanges.push({
        projectId,
        prCount: projectDirectional.length,
        signals: uniqueStrings(projectDirectional.map((pr) => pr.directionSignal)),
      });
    }
  ...
  return {
    ...
    dailyDigestCoverage: getDailyDigestCoverage(db, startUnix, endUnix),
    directionChanges,
    counterpartChecks: [],
    projects,
  };
...
export function renderWeeklyPrompt(template: string, input: WeeklyPromptInput): string {
  const inputJson = JSON.stringify(input, null, 2);
  const replacements: Record<string, string> = {
    WEEKLY_INPUT_JSON: inputJson,
  ...
  if (!template.includes("{{WEEKLY_INPUT_JSON}}")) {
    rendered += `\n\nWEEKLY_INPUT_JSON:\n${inputJson}`;
  }
